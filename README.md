...
- **中途插話**：agent 忙碌時仍可送訊息 — 直接送＝排隊（當前回合結束才跑），⏭ 插話＝打斷當前回合並立即處理
...
- **排隊送出（↑）**：訊息送出後會排在當前回合後面，等它結束再依序處理（保留當前工作）；泡泡上標「排隊中」，開始執行時自動消失。leader 會廣播佇列狀態，多裝置與終端機同步可見。
- **插話（⏭）**：打斷（cancel）當前回合並立即送出你的訊息，馬上處理。適合「講錯了、改方向、先做這個」。
- **停止（■）**：只取消當前回合，不送新訊息。

> 註：grok 的 stdio ACP 不開放直接編輯佇列（`queue/interject`、`queue/remove` 是 agent→client 廣播），所以「插話」以 `session/cancel` + `session/prompt` 實作。
